...
## 2026-08-03 (5일차) — 대시보드: 루트 URL이 살아있는 그래프가 되다

- **★ 대시보드 배포 완료**: http://13.124.60.61:8080/ — 정적 HTML 1장(CSS·JS 인라인, 외부 의존 0). Pencil 시안 A/B 비교 후 A안(shadcn 언어) 채택, brainstorming→스펙→구현계획→인라인 실행 풀사이클.
- **구성**: 실측 카드 3(1/5,044·2/2·22.4ms) + SVG 타임라인 차트(검출 음영·score·크로스헤어 툴팁) + 집계 콘텐츠만 드롭다운(193편 프로브→5편) + 하이라이트 표 + **실시간 응답속도 배지**(첫 조회 434ms 미스 → 이후 히트가 수치로 보임).
- **프리즈 예외 1건 (승인)**: SecurityConfig permitAll에 `"/","/index.html"` — 루트 403을 구현 중 발견, 스펙에 기록. 테스트 34개 green 유지.
- **계획↔실제 차이 2건**: `/api/contents`가 Page 객체+`contentId` 필드(계획은 `id` 가정) → 실응답 대조로 수정. EC2 시연 데이터 콘텐츠 2~5 확충(각 하이라이트 2개).
- **트러블슈팅 8호**: 재배포가 "안 끝나는" 증상 — 스래싱·재부팅 무시·CPU 크레딧 3개 가설 전부 기각 끝에 진범은 **EIP 주소 변경 후 SSH 호스트 키 프롬프트 무한 대기**(stdin 닫힌 자동화 환경). TCP:22 도달 테스트가 결정타. `StrictHostKeyChecking=accept-new`로 해결. 상세: `troubleshooting/2026-08-03-ssh-hostkey-silent-hang.md`
- **검증 (EC2 외부)**: 루트 200·드롭다운 5편·SVG 차트·표 2행·배지 동작·콘솔 에러 0·Swagger 200·health UP.
- **남은 것**: 서류만 (사용자 주도). 8/6 이후 서버 동결.
...
- **★ 결과**:
  ...
  합산: TP(검출)=184, FP=398, 발견=167/170 — 정밀도 31.6%, 재현율 98.2%
- TP(184) > 발견(167)인 이유: 넓은 피크가 여러 검출 창으로 쪼개져 각각 TP로 집계되기 때문 (병합 비대칭의 역방향 — 채점 규칙상 정상)
- **발견**: FP 398건 중 314건(구조 1·2, 무피크)·63건(구조 4, 약한 피크 ×4)·21건(구조 12, 완만 피크 ×3)이 전부 — 강한 피크가 있는 구조(3·5·6·7·8·9·10·11)는 전부 FP 0. 원인은 z-score의 상대성: 강한 피크가 있으면 σ가 부풀어 노이즈가 z≥2.0을 못 넘지만, 피크가 없거나 약하면 σ가 작아 순수 노이즈도 넘는다 — 절대 높이 하한이 없는 게 구조적 원인. 기존 시연(2/2)에서 FP가 안 보였던 이유도 이것으로 설명된다: 시연 콘텐츠가 전부 강한 피크였다. 놓친 3개는 전부 구조 8(강+약 혼합)의 약한 피크 — 강한 피크의 σ 부풀림이 같은 콘텐츠의 약한 피크를 가림(day3에 기록된 σ 부풀림과 같은 기제지만 양상은 다름 — 그때는 짧은 시계열에서 피크가 자기 z-score를 깎았고, 이번엔 다른 피크를 가림. 근거 주석: HighlightDetectorTest). 약한 피크 단독(구조 4)은 10/10 발견 — σ가 작아서 ×4도 넘지만, 같은 이유로 FP 63건도 함께 생겼다.
- **원칙**: z=2.0 유지 — 측정이 튜닝이 되지 않도록 임계값은 건드리지 않고 한계로 기록.
  실측값은 assert로 고정 → 이후 상수 변경 시 회귀 테스트가 잡는다.
- **환경 이슈**: `./gradlew test` 실행 중 `ScenelogApplicationTests.contextLoads()`가 Hibernate dialect 오류로 실패 — 로컬 PostgreSQL(docker-compose)이 안 떠 있어서다. Docker Desktop이 1호와 같은 계열의 손상된 소켓 버그(`dockerInference`, 이번엔 `docker-secrets-engine`)로 기동 실패 + 자동 업데이터까지 걸려 이번 세션에서는 복구하지 못함. 이 테스트는 이번 태스크가 추적하는 40개(기존 34 + 신규 6)에 포함되지 않고, 이번 변경(GoldenSetEvalTest·dev-log)과 무관 — 40개는 전부 green. 로컬 DB 기동 후 재검증 필요.
...
## 2026-08-09 (8/9 추가) — 검출기 v2: 절대 하한(MIN_LIFT)으로 정밀도 개선
...
- **★ 상수 선정 (튜닝 시드 1~10 스윕)**:
  ...
  | k | TP | FP | 발견/정답 | 정밀도 | 재현율 |
  |---|---|---|---|---|---|
  | 0.00 | 184 | 398 | 167/170 | 31.6% | 98.2% |
  | 1.25 | 184 | 398 | 167/170 | 31.6% | 98.2% |
  | 1.50 | 184 | 398 | 167/170 | 31.6% | 98.2% |
  | 2.00 | 192 | 60 | 167/170 | 76.2% | 98.2% |
  | 2.50 | 194 | 4 | 167/170 | 98.0% | 98.2% |
  | 3.00 | 167 | 1 | 162/170 | 99.4% | 95.3% |

  기준 "발견 ≥165(재현율 ≥97%) 유지 중 정밀도 최대" 적용: k=3.00은 발견 162<165(재현율 95.3%)로
  탈락, k=0.00~2.50은 전부 발견 167로 통과 — 통과군 중 정밀도 최대는 k=2.50(98.0%), 단독
  최댓값이라 동률 규칙(작은 k 우선) 불필요. **선정 k = 2.5**
- **★ 공식 결과 (검증 시드 11~20, 한 번도 안 본 데이터)**: v1 31.6%/98.2% →
  **v2 98.5%/97.6%** (TP=195, FP=3, 발견=166/170)
  ...
  | 11 고노이즈 | 10/10 | 0 |
  | 12 완만 5분 | 10/10 | 0 |
- **운영 노트**: ① ZSCORE_V2 전환 후 재집계 전까지 `/highlights` 조회가 빈 목록을 반환한다 —
  조회가 `method='ZSCORE_V2'` 행만 찾는데 배포 시점 DB엔 아직 V1 행뿐이라서다. 마이그레이션
  누락이 아니라 "V1 이력 보존"이 의도된 설계이므로, 배포 시 반드시 재집계(AggregationService
  재실행)가 먼저 돌아야 한다. ② 파생 쿼리 `findByContentIdAndMethodOrderByStartSec`는 이번
  세션 내내 Docker Desktop 비가동으로 실제 JPQL 부팅 검증(contextLoads)을 못 했다 — 컴파일은
  성공(정적 시그니처 확인)했지만 런타임 파싱은 미확인. Docker 복구 후
  `./gradlew test --tests "com.scenelog.ScenelogApplicationTests"` 1회 필요.
- **남은 한계**: 검증 시드에서도 구조 8(강+약 혼합)은 여전히 4개 미검출(16/20) — 강한 피크의
  σ 부풀림이 같은 콘텐츠의 약한 피크를 가리는 문제는 절대 하한(minLift)으로는 못 고친다
  (median/MAD 후속 과제). 잔여 FP 3건은 구조 2(무피크·긴) 2건 · 구조 4(약한 피크 1개) 1건 —
  전부 무피크·약한 신호 근처로 v1 FP 398건과 같은 계열이지만 규모가 세 자리에서 한 자리로 줄었다.
